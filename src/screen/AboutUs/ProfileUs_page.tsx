...
import ProfileWhatScreen from './Component/profile_what.page';
import ProfileVisionScreen from './Component/profile_vision_page';
import colors from '../../config/colors';
import {Scene, Event} from 'react-native-tab-view/lib/typescript/src/types';

const renderScene = SceneMap({
  first: ProfileWhatScreen,
  second: ProfileVisionScreen,
});

const ProfileUsScreen: React.FC = () => {
  const layout = useWindowDimensions();

  const [index, setIndex] = React.useState(0);
  const [routes] = React.useState([
    {key: 'first', title: 'What is ICSA?'},
    {key: 'second', title: 'Vision & Mission'},
  ]);
  const renderTabBar = (
...
export default ProfileUsScreen;
